...
PicoKeypad::PicoKeypad(char *keymap, uint8_t *rowPins, uint8_t *columnPins, uint8_t numRows, uint8_t numCols) {
    _keymap = keymap;
    _row_pins = rowPins;
    ...
    for (int col = 0; col < _num_cols; col++) {
        ...
        for (int row = 0; row < _num_rows; row++) {
            if (gpio_get(_row_pins[row]) == 0) {
                ...
                if ((_prev_key != key) || (_prev_key == key && (now - _prev_time) > _debounce_time)) {
                    _prev_key = key;
                    _prev_time = now;
                    gpio_put(_column_pins[col], 1);
                    return key;
...
void PicoKeypad::setDebounceTime(uint32_t time) {
    _debounce_time = time;
}
